...
logger = logging.getLogger("run_aifsv2")


def get_ic(date: datetime):
    ic_filename = root / "IC_data" / f"{date.strftime('%Y%m%d')}" / f"input_state_{date.strftime('%Y%m%dT%H')}_v2.pkl"
...
def run(date: datetime, lead_time: int, save_fields: list):
    ...
    os.makedirs(output_dir, exist_ok=True)
    output_filename = output_dir / f"init_v2_{date.strftime('%Y%m%dT%H')}.nc"
...
def main():
    parser = argparse.ArgumentParser(description="Run AIFS v2")
    ...
    try:
        ...
        logger.info("AIFS v2 run completed successfully.")
        sys.exit(0)
